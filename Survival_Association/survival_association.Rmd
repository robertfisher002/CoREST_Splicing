...
common_splice <- read.csv("/Users/rjfisher/Library/Mobile Documents/com~apple~CloudDocs/BUSM/Alani Lab/DATA/CoREST Splicing/rmats/processed_rmats/overlap/CommonSpliceEvent.csv")
...
hr_data <- spladder_common_exons[, c("Gene_Symbol", "Hazard_Ratio", "Pvalue_HR", "IncLevelDifference")]

# Filter out NA p-values
hr_data <- hr_data[!is.na(hr_data$Pvalue_HR), ]

# Create a dotplot
ggplot(hr_data, aes(x = Hazard_Ratio, y = Gene_Symbol, size = -log10(Pvalue_HR), color = ifelse(IncLevelDifference > 0, "Excluded", "Included"))) +
  geom_point() +
  geom_vline(xintercept = 1, linetype = "dashed", color = "black") +
  labs(x = "Hazard Ratio", y = "Gene Symbol", size = "-log10(P-value)", color = "IncLevelDifference") +
  theme_bw()
